...
2. Run Migration to setup Database
   ```bash
   npm run db:migrate
   ```

3. Start the Backend Server (dev mode)
   ```bash
   npm run server:dev
   ```
4. Seed the database for initial data (users, organizations and stores) to work with:
   ```bash
   npm run db:seed
   ```
...
6. Access services:
   ...
   - Backend (Static React App): `http://localhost:3000/` *(Requires `npm run build` command to reflect changes)*
   - Backend (REST APIs): `http://localhost:3000/api`
   - Adminer: `http://localhost:8080`

7. To create new migration (when changes in DB schema are needed):
   ```bash
