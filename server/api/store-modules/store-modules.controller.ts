...
export const getStoreModules = asyncHandler(
...
export const initializeStoreModules = asyncHandler(
  async (req: Request, res: Response) => {
    const { storeId } = req.params;
    ...
    // Transform the response
    const transformedModules = modules.map(
      ({ module, isEnabled, stats, Devices }) => ({
        id: module.id,
        name: module.name,
        key: module.key,
        isEnabled,
...
export const getModuleWithDevices = asyncHandler(
  async (req: Request, res: Response) => {
    const { storeId, moduleId } = req.params;
    ...
    const storeModule = await storeModuleService.getModuleWithDevices(
      storeId,
      moduleId
    );
