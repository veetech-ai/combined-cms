...
const configSchema = z.object({
  ...
  baseUrl: z.string().url().default('http://localhost:4000/api/v1'),
  nodeEnv: z.enum(['development', 'production', 'test']).default('development'),
  cloverBaseUrl: z.string().url().default('https://sandbox.dev.clover.com/v2/'),
  db: z.object({
  ...
  clover: z.object({
    eCommerceServiceUrl: z.string().url().default('https://scl.clover.com/v1'),
    eCommercePrivateToken: z
      .string()
      .default('93b89475-c488-b77e-c48d-84a50c36c4a4')
  }),
  jwt: z.object({
...
const env = {
  ...
  clover: {
    eCommerceServiceUrl: process.env.CLOVER_ECOMMERCE_SERVICE_URL,
    eCommercePrivateToken: process.env.CLOVER_ECOMMERCE_PRIVATE_TOKEN
  },
  db: {
