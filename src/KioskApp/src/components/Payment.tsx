...
import { Star, Instagram, ArrowLeft, Check, TagIcon as TacoIcon } from 'lucide-react';



export function Payment() {
    const customerName = "Shariq";
    const items = [
        { quantity: 1, name: 'Chicken Karahi Rice Bowl', price: 15.00 },
        { quantity: 1, name: 'Beef Nihari Tacos (2)', price: 10.00 },
        { quantity: 1, name: 'Chicken Karahi Burrito', price: 15.00 },
        { quantity: 1, name: 'Chicken Karahi Tacos (2)', price: 9.00 },
        { quantity: 1, name: 'Aloo Tacos (2)', price: 9.00 },
        { quantity: 1, name: 'Side Naan', price: 4.00 }
      ];
    
      const amount = 62.00; // Total amount
  const [currentScreen, setCurrentScreen] = useState<'apple' | 'google' | 'processing' | 'confirmation'>('apple');
  const [rating, setRating] = useState<number>(0);
  const [feedbackSubmitted, setFeedbackSubmitted] = useState(false);
  const [isAnimating, setIsAnimating] = useState(false);
  const orderId = "5G5NDM6SPLT0F";
  ...
  const renderApplePayScreen = () => (
    <div className="flex flex-col h-full bg-[#EEEEEE] animate-fadeIn relative">
      ...
      <div className="flex-1 p-8 text-left">
        ...
        <div className="text-6xl font-bold mb-8 text-black tracking-tight">${amount.toFixed(2)}</div>
        
        <div className="mb-8">
          <div className="h-px bg-gray-200 w-full mb-8" />
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium text-gray-500">ORDER SUMMARY</h3>
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-[#06C167] rounded-full flex items-center justify-center">
                <span className="text-white font-medium">{customerName.charAt(0)}</span>
              </div>
              <span className="text-lg text-gray-700 font-medium">{customerName}</span>
            </div>
          </div>
        </div>
      ...
      <div className="p-6 space-y-4 bg-white shadow-xl rounded-t-[2.5rem] fixed bottom-0 left-0 right-0 max-w-lg mx-auto">
        <button 
          ...
          className="w-full bg-black text-white text-xl font-bold rounded-2xl py-5 flex items-center justify-center gap-3 shadow-lg transition-all hover:bg-gray-800 hover:shadow-xl active:transform active:scale-98"
        >
          Pay with Apple Pay
        </button>
        <button 
  ...
  const renderGooglePayScreen = () => (
    <div className="flex flex-col h-full bg-[#EEEEEE] animate-fadeIn relative">
      ...
      <div className="flex-1 p-8 text-left">
        <h2 className="text-2xl font-bold mb-1 font-roboto text-black">Total Amount</h2>
        <div className="text-6xl font-bold mb-8 font-roboto text-black tracking-tight">${amount.toFixed(2)}</div>
        
        <div className="mb-8">
          <div className="h-px bg-gray-200 w-full mb-8" />
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium text-gray-500 font-roboto">ORDER SUMMARY</h3>
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-[#06C167] rounded-full flex items-center justify-center">
                <span className="text-white font-medium">{customerName.charAt(0)}</span>
              </div>
              <span className="text-lg text-gray-700 font-medium font-roboto">{customerName}</span>
            </div>
          </div>
        </div>
          ...
          {items.map((item, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <span className="text-xl font-medium font-roboto">{item.name}</span>
              </div>
              <span className="text-xl font-bold font-roboto">${item.price.toFixed(2)}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  const renderProcessingScreen = () => (
    <div className="flex flex-col h-full bg-[#EEEEEE] animate-fadeIn">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        ...
        <h2 className="text-4xl font-bold mb-6 text-center">Processing Payment</h2>
        <p className="text-2xl text-gray-600 text-center max-w-sm">
          Securely processing your payment of <span className="font-semibold text-black">${amount.toFixed(2)}</span>
        </p>
  ...
  const renderConfirmationScreen = () => (
    <div className="flex flex-col h-full bg-[#EEEEEE] animate-fadeIn relative">
      <div className="flex-1 flex flex-col items-center justify-center p-6 space-y-6 overflow-y-auto pb-[100px]">
        ...
        <div className="text-6xl font-bold tracking-tight">${amount.toFixed(2)}</div>
        <p className="text-xl text-gray-600 text-center">We'll notify you when your order is ready.</p>

        {!feedbackSubmitted ? (
          <div className="w-full max-w-md bg-white p-6 rounded-3xl shadow-xl animate-fadeIn">
            <h3 className="text-2xl font-medium text-center mb-4">Rate your experience</h3>
          ...
          <div className="w-full max-w-md bg-white p-6 rounded-3xl shadow-xl animate-fadeIn">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-medium mb-4">Thank you for your rating!</h3>
